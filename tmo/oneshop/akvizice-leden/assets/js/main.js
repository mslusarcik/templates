function checkOneshopScripts() {
  ...
  function openModalWindow(mainObj){
    ...
    console.log("Modal found: " + modals.length);
    if (modals) {
      for (var l = 0; l < modals.length; l++) {
        ...
        modals[l].onclick = function (e) {
          ...
          console.log(modal);
          var modalCloseBtn = modal.querySelectorAll(".modal-close-button")[0];
   ...
   * FUNCTION
   * ***************** */
  function scrollAndSetTab(mainObj){
    var url_string = (window.location.href).toLowerCase();
    console.log(url_string);
    var url = new URL(url_string);
    console.log(url);
    var openTabValue = url.searchParams.get("opentab");
    console.log(openTabValue);
    if(openTabValue){
      var anchor = mainObj.querySelector(".tabs");
      anchor.scrollIntoView({
        behavior: "auto",
        block: "start"
      });
      mainObj.querySelector('[data-tab="'+ openTabValue +'"]').click();
    }
  }
